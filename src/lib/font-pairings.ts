 ...
 * Font Pairing System
 * 
 * This module provides seamless font pairing integration for the application.
 * To change the font pairing, simply update the SELECTED_FONT_PAIRING constant in main.ts
 * 
 * Available pairings:
 * - reddit-sans-sora
 * - arvo-geist  
...
const fontPairings: Record<FontPairingKey, FontPairing> = {
  'reddit-sans-sora': {
...
export class FontPairingManager {
  private currentPairing: FontPairingKey = 'mozilla-fonts'; // Default pairing

  // Function to load fonts and apply styles
  loadFontPairing(pairingKey: FontPairingKey): void {
  ...
  getAvailablePairings(): FontPairingKey[] {
    return Object.keys(fontPairings) as FontPairingKey[];
  }

  // Get current pairing info
  getCurrentPairing(): FontPairing & { key: FontPairingKey } {
    return {
  ...
  // Set current pairing
  setCurrentPairing(pairingKey: FontPairingKey): void {
    this.loadFontPairing(pairingKey);
  }
}

// Export a default font manager instance
export const fontManager = new FontPairingManager();

// Export the font pairings for direct access
export { fontPairings };
